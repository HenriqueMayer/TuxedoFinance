...
class PlanningPositionTests(TestCase):
    ...
    def test_section_scopes_positions_history_and_totals_and_boosted_navigation_is_a_page(self):
        ...
        self.assertNotIn('paginator', portfolio.context)
        history = self.client.get(reverse('investments:operations'), {'section': 'portfolio'})
        self.assertEqual(history.context['paginator'].count, 0)
        ...
        self.assertNotIn('paginator', pots.context)
        history = self.client.get(reverse('investments:operations'), {'section': 'cash'})
    ...
    def test_operations_filter_purpose_and_keep_other_users_private(self):
        cash = InvestmentProduct.objects.create(user=self.user, bank=self.bank, name='Cash pot', purpose='MONTHLY_CASH')
        asset = self.asset()
        for product, reason in [(self.product, 'Portfolio yield'), (cash, 'Pot yield')]:
            Investment.objects.create(user=self.user, product=product, asset=asset, kind='YIELD', amount=Decimal('10'), date=timezone.localdate(), reason=reason)
        response = self.client.get(reverse('investments:operations'))
        self.assertEqual(response.context['paginator'].count, 2)
        response = self.client.get(reverse('investments:operations'), {'section': 'cash', 'q': 'Pot', 'kind': 'YIELD', 'bank': self.bank.pk, 'asset': asset.pk})
        self.assertEqual(response.context['paginator'].count, 1)
        self.assertContains(response, 'Pot yield')
        self.assertNotContains(response, 'Portfolio yield')
        self.assertNotIn('chart_total', response.context)
        other = get_user_model().objects.create_user('other-operation-owner')
        self.client.force_login(other)
        response = self.client.get(reverse('investments:operations'), {'product': cash.pk})
        self.assertEqual(response.context['paginator'].count, 0)
        self.assertNotContains(response, 'Cash pot')
        self.client.logout()
        self.assertEqual(self.client.get(reverse('investments:operations')).status_code, 302)

    def test_old_operation_bookmarks_redirect_with_their_scope(self):
        for section in ('portfolio', 'cash'):
            response = self.client.get(reverse('investments:list'), {'section': section, 'q': 'Yield'})
            self.assertRedirects(response, reverse('investments:operations') + f'?section={section}&q=Yield')
        response = self.client.get(reverse('investments:list'), HTTP_HX_REQUEST='true', HTTP_HX_TARGET='investment-movements', follow=True)
