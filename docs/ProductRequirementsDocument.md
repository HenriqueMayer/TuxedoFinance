...
> **Scope change (2026-08-02) — theme toggle:** FR21 adds a light/dark theme toggle. The toggle carries ~25 lines of vendored vanilla JS (`static/js/theme.js` + the matching inline `<head>` FOUC script in `base.html`). That is **not** the "JavaScript build step" the §5 exclusion above was protecting against — there is still no npm/Node pipeline, no SPA, and no charting library. HTMX is used narrowly for partial swaps on Reports and Investments, with a shared helper that preserves viewport/focus; charts remain server-rendered SVG/CSS and every mutating action stays a normal Django POST + redirect. The theme and HTMX scripts are the project's deliberately scoped JavaScript exceptions.
