...
Three screens over the same data: the **index** (six aggregate indicators and a forward-looking outlook table), **reports** (six charts of how the account evolves over a year — the time-series line + cash-flow bars share a 12-month window, a recurrence donut is third, and two payment-method breakdowns plus a category breakdown follow with their own scoped filters), and the per-method **category breakdown** that opens inline on the reports page when a bar in charts 4 or 5 is clicked. The index is `LOGIN_REDIRECT_URL` — the first screen every user sees after signing up or logging in. Transactions are intentionally reviewed on the dedicated Transactions screen instead of being duplicated on the index.

Both the index and reports are a **forecast**, not a report of the past: the index uses `?month=YYYY-MM` for month navigation, and fixed transactions and open installment plans recur into future months automatically. The reports page uses `?charts_offset=N` to slide the 12-month time-series window, `?installment_month=ALL|YYYY-MM` to scope the third-position recurrence donut (defaults to the current month), `?payment_month=ALL|YYYY-MM` to scope both payment-method charts at once, `?expense_method=NAME` / `?income_method=NAME` to open each inline method-categories drill-down (charts 4 and 5 respectively), and `?category_month=ALL|YYYY-MM` to scope the final "where the money goes" chart.
...
The third chart on the reports page: expenses split into three buckets by how they recur, answering "how much of *this* month's outflow is from parcels I bought before".
...
The geometry behind chart 3 (`get_expenses_by_recurrence`). Same "pure geometry, no money/Django knowledge" contract as the line and bar builders, but for a donut/ring instead of a Cartesian chart.
...
It reads **six independent query params** off `request.GET`, all with the same forgiving contract (malformed → silently fall back to the default rather than raise a 400):
...
| `?payment_month=ALL\|YYYY-MM` | `ALL` | `_is_representable` | Which month (or all of them) charts 4 and 5 fold — the two payment-method breakdowns share one window on purpose. |
| `?expense_method=NAME` | unset | validated against `expense_breakdown['methods']` (stale URLs cannot surface an empty panel) | Opens the inline "categories for this method" panel below chart 4 (spending). |
| `?income_method=NAME` | unset | validated against `income_breakdown['methods']` | Opens the inline "categories for this method" panel below chart 5 (income). |
| `?category_month=ALL\|YYYY-MM` | `ALL` | `_is_representable` | Which month (or all of them) the "where the money goes" chart (6) folds. |
| `?installment_month=ALL\|YYYY-MM` | **current month** | `_is_representable` | Which month (or all of them) the recurrence donut (chart 3) folds. Defaults to the current month, not `ALL`, because the chart answers "how much of *this* month is on installments". Parsed by the view's own `get_installment_month` method, which is the only param here that does not reuse `_parse_month_or_all` directly — it intercepts the empty param first and returns today's `YYYY-MM` before handing off to the shared parser. |
...
The params **do not interact**: a user can offset the time-series window by 2 months while keeping the breakdown chart scoped to March 2026 and the recurrence donut to July 2026, and clicking a bar in chart 4 preserves every active filter. The template uses `{% querystring %}` everywhere so any one of them survives navigation to any other.

When `?expense_method=NAME` matches a method in the current spending breakdown, the view computes `expense_method_categories` (top categories for that method in the same window chart 4 is showing) and surfaces it in the inline panel below chart 4. `?income_method=NAME` does the same on the income side against chart 5, validated against the income breakdown so a stale URL cannot open a panel for a method that isn't on the chart it was clicked from. If `?payment_month=ALL`, both panels anchor on the current month — otherwise they follow the chosen month. A missing param, an unknown name, or a name from a stale URL is silently ignored and the panel stays hidden.

Chart 3 (`get_installment_month` / `recurrence_breakdown` / `recurrence_chart`) lives in the same partial. The view always computes the recurrence breakdown for the resolved window — even when all three buckets are zero — so the legend keeps rendering all three entries with `share=0`. `recurrence_chart` is set to `None` only when `recurrence_breakdown['slices']` itself is empty (`get_expenses_by_recurrence` returns `{'slices': []}` for a window with no expenses at all), at which point the template renders the `empty_state.html` partial instead of a blank SVG.
...
5. **Income by payment method** — the same bar layout as chart 4, opposite direction of cash (emerald). Each bar hyperlinks to `?income_method={name}#method-categories` and opens its own inline panel below; the two drill-downs are independent so the user can have one open on each side at the same window.
6. **Where the money goes** — plain CSS bars (`style="width: {{ category.bar_width|unlocalize }}%"`), no SVG needed.
...
It is a `TemplateView`, not a `ListView`: the index composes aggregate indicators and the outlook returned by `get_dashboard_summary`; the primary paginated transaction resource remains `transactions:list`. The former recent-transactions widget was removed so transaction review happens in that dedicated screen and the index does not execute a second transaction query solely for a duplicate list.
